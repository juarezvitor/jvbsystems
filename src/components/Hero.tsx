...
export default function Hero() {
  return (
    <section className="relative min-h-screen flex flex-col items-center justify-center
             ...
             style={{background:'var(--bg-base)'}} aria-label="Seção principal">
      ...
      {/* Badge */}
      <div className="relative z-10 inline-flex items-center gap-2 mb-8 reveal"
           style={{
             background:'rgba(0,196,122,0.09)',
             border:'1px solid rgba(0,196,122,0.25)',
             padding:'6px 16px', borderRadius:999,
             color:'var(--green-dark)', fontSize:11, fontWeight:600, letterSpacing:'0.08em', textTransform:'uppercase',
           }}>
        <span className="w-1.5 h-1.5 rounded-full pulse-dot" style={{background:'var(--green)'}} aria-hidden="true"/>
        Automações &amp; Desenvolvimento Web
      </div>

      {/* Logo grande centralizada */}
      <div className="relative z-10 mb-6 reveal">
        <Image src="/logo.png" alt="JVBSystems" width={96} height={96}
          className="object-contain mx-auto drop-shadow-sm" priority/>
      </div>

      {/* Headline */}
      <h1 className="relative z-10 font-display font-extrabold leading-[1.03] tracking-tight mb-6 reveal"
          style={{color:'var(--text-primary)', fontSize:'clamp(44px,8vw,88px)'}}>
        Tecnologia que<br/>
        <span className="gradient-text">transforma</span><br/>
        o seu negócio
      </h1>

      <p className="relative z-10 font-light max-w-[500px] leading-relaxed mb-10 reveal"
         style={{color:'var(--text-secondary)', fontSize:'clamp(15px,1.8vw,18px)'}}>
      ...
      <div className="relative z-10 flex flex-wrap gap-3 justify-center mb-20 reveal">
        ...
        <Link href="#servicos"
          ...
          onMouseEnter={e=>(e.currentTarget.style.borderColor='rgba(0,196,122,0.4)')}
      ...
      {/* Stats bar */}
      <div className="relative z-10 stat-bar flex w-full max-w-[420px] reveal"
        ...
        {[
          ...
          <div key={s.v} className="flex-1 flex flex-col items-center py-5 px-3 text-center">
            <span className="font-display font-bold text-lg mb-0.5" style={{color:'var(--navy)'}}>{s.v}</span>
      ...
      {/* Scroll hint */}
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-1.5 opacity-40" aria-hidden="true">
        <span className="text-[10px] font-medium tracking-widest uppercase" style={{color:'var(--text-muted)'}}>Scroll</span>
        <svg width="16" height="20" viewBox="0 0 16 20" fill="none">
          <rect x="1" y="1" width="14" height="18" rx="7" stroke="currentColor" strokeWidth="1.2" style={{color:'var(--text-muted)'}}/>
          <circle cx="8" cy="7" r="2" fill="currentColor" style={{color:'var(--green)', animation:'scrollDot 1.8s ease-in-out infinite'}}/>
        </svg>
      </div>
    </section>
  )
